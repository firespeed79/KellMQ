...
{
    [Serializable]
    public class ClonableCommunicationClient : IDisposable, ISubscribe
    {
        Client<BroadcastEvent> send;
        int port = 8888;
        ClientType ct;

        public event ClonableSubscribeHandler Subscribe;
        private IPAddress server;

        public ClonableCommunicationClient(ClientType ct, string serviceName)
        {
            this.ct = ct;
        ...
        public ClonableCommunicationClient(ClientType ct, int port, string serviceName)
        {
            this.ct = ct;
            this.port = port;
            send = new Client<BroadcastEvent>(IPEP, serviceName);
        ...
        public ClonableCommunicationClient(ClientType ct, IPAddress server, int port, string serviceName)
        {
            this.ct = ct;
            this.server = server;
            this.port = port;
        ...
        public bool Send<T>(T msg) where T : ClonableObject
        {
            if (send.SharedObject != null)
            {
                List<IPAddress> ips = KellMQ.Common.GetIPsV4();
                if (ips.Count > 0)
                {
                    send.SharedObject.Broadcast(new KellMQ.BroadcastEventArgs(new IPEndPoint(ips[0], port), KellMQ.Common.GetDataClonable<T>(ct, msg)));
                    return true;
                }
                else
                {
        ...
        public void SubscribeHanler(object sender, BroadcastEventArgs e)
        {
            if (Subscribe != null)
            {
                ClonableObject msg = Common.GetMsgClonable<ClonableObject>(e.Data);
